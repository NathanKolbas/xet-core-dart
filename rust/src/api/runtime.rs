...
use tracing::info;
pub use xet_runtime::XetRuntime;
pub use xet_runtime::errors::MultithreadedRuntimeError;
...
#[frb(ignore)]
pub fn convert_multithreading_error(e: impl Into<MultithreadedRuntimeError> + std::fmt::Display) -> String {
    format!("Xet Runtime Error: {e}")
}
...
{
    // Spawn the task on a new OS thread. This avoids having tokio cache stuff in
    // thread-local storage that is invalidated after a fork-exec.
    let result: Result<Out, String> = spawn_os_thread(move || {
